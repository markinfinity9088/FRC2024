...
public class CANSparkMaxSim1 {
  ...
  private boolean runLimitLogic(boolean forward) {
    if (forward) {
      if ((m_sparkMax.getSoftLimit(SoftLimitDirection.kReverse) > m_position.get())
          && m_sparkMax.isSoftLimitEnabled(SoftLimitDirection.kForward)
          ) {
        return true;
      }

      return (m_sparkMax.isSoftLimitEnabled(SoftLimitDirection.kForward));
    } else {
      if ((m_sparkMax.getSoftLimit(SoftLimitDirection.kReverse) > m_position.get())
          && m_sparkMax.isSoftLimitEnabled(SoftLimitDirection.kReverse)) {
        return true;
      }

      return (m_sparkMax.isSoftLimitEnabled(SoftLimitDirection.kReverse));
    }
  }
  ...
  public void iterate(double velocity, double vbus, double dt) {
    ...
    //KP commented below and added positionFactor to 1 as I am getting compiler errors due to protected method
